...
public class PaletteScreen extends Screen {
    public static final ResourceLocation BACKGROUND = ResourceLocation.fromNamespaceAndPath(Nekoration.MODID, "textures/gui/palette.png");
    ...
    public void onClose() {
        try {
            ...
            final var packet = new PaletteUpdatePayload(this.hand, activeSlot, cls);
            NekoPlatform.sendToServer(packet);
        } catch (Exception e) {
            e.printStackTrace();
        }
        super.onClose();
    ...
    public void render(GuiGraphics graphics, int x, int y, float partialTicks) {
        ...
        graphics.blit(BACKGROUND, i + 8 + 18 * activeSlot + (activeSlot > 2 ? 34 : 0), j + 13, 172, 16, 16, 16); // Slot Indicator...
        // Step 4: Render the color map...
        graphics.pose().pushPose();
        graphics.pose().mulPose(Axis.ZP.rotationDegrees(90.0F));
        graphics.pose().translate(j + 32, -i - 137, 0);
        graphics.fillGradient(0, 0, 128, 128, colorMapColor.getRGB(), white);
        graphics.pose().popPose();
        graphics.fillGradient(i + COLORMAP_LEFT, j + COLORMAP_TOP, i + COLORMAP_LEFT + 128, j + COLORMAP_TOP + 128, 0, black);
    ...
    protected void renderBg(GuiGraphics graphics) {
        ...
        graphics.blit(BACKGROUND, edgeSpacingX, edgeSpacingY, 0, 0, this.imageWidth, this.imageHeight);
    }
