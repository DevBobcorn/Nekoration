...
import io.devbobcorn.nekoration.client.rendering.PaintingRendererManager;
...
public class PaintingRenderer extends EntityRenderer<PaintingEntity> {
    ...
    public void render(PaintingEntity entity, float rotation, float partialTicks, PoseStack stack, MultiBufferSource buffers, int packedLight) {
        ...
        renderPainting(stack, buffers, entity, entity.getWidth() * 16, entity.getHeight() * 16, paintingspriteuploader.getBackSprite());
        stack.popPose();
    ...
    private void renderPainting(PoseStack stack, MultiBufferSource buffers, PaintingEntity entity, int width, int height, TextureAtlasSprite woodTex) {
        ...
        boolean fastRender = NekoPlatform.config().simplifyRendering();
        boolean useImageRendering = NekoPlatform.config().useImageRendering();

        boolean renderWithImage = entity.data.imageReady && useImageRendering;

        int entityLight = LevelRenderer.getLightColor(entity.level(), entity.blockPosition());
        ...
        for (short blocHor = 0; blocHor < blocHorCount; ++blocHor) { // BlockCount Horizontally...
            for (short blocVer = 0; blocVer < blocVerCount; ++blocVer) { // BlockCount Vertically...
                ...
                // Pos[Z] // B[ack]
                vertexFrame(pose, vb, right, top, woodU0, woodV0, 0.5F, 0, 0, 1, light);
                vertexFrame(pose, vb, left, top, woodU1, woodV0, 0.5F, 0, 0, 1, light);
                vertexFrame(pose, vb, left, bottom, woodU1, woodV1, 0.5F, 0, 0, 1, light);
                vertexFrame(pose, vb, right, bottom, woodU0, woodV1, 0.5F, 0, 0, 1, light);
                // Pos[Y] // U[p]
                if (blocVer == blocVerCount - 1) {
                    vertexFrame(pose, vb, right, top, woodU0, woodV0, -0.5F, 0, 1, 0, light);
                    vertexFrame(pose, vb, left, top, woodU1, woodV0, -0.5F, 0, 1, 0, light);
                    vertexFrame(pose, vb, left, top, woodU1, woodV_, 0.5F, 0, 1, 0, light);
                    vertexFrame(pose, vb, right, top, woodU0, woodV_, 0.5F, 0, 1, 0, light);
                }
                // Neg[Y] // D[own]
                if (blocVer == 0) {
                    vertexFrame(pose, vb, right, bottom, woodU0, woodV0, 0.5F, 0, -1, 0, light);
                    vertexFrame(pose, vb, left, bottom, woodU1, woodV0, 0.5F, 0, -1, 0, light);
                    vertexFrame(pose, vb, left, bottom, woodU1, woodV_, -0.5F, 0, -1, 0, light);
                    vertexFrame(pose, vb, right, bottom, woodU0, woodV_, -0.5F, 0, -1, 0, light);
                }
                // Neg[X] // L[eft]
                if (blocHor == blocHorCount - 1) {
                    vertexFrame(pose, vb, right, top, woodU_, woodV0, 0.5F, -1, 0, 0, light);
                    vertexFrame(pose, vb, right, bottom, woodU_, woodV1, 0.5F, -1, 0, 0, light);
                    vertexFrame(pose, vb, right, bottom, woodU0, woodV1, -0.5F, -1, 0, 0, light);
                    vertexFrame(pose, vb, right, top, woodU0, woodV0, -0.5F, -1, 0, 0, light);
                }
                // Pos[X] // R[ight]
                if (blocHor == 0) {
                    vertexFrame(pose, vb, left, top, woodU_, woodV0, -0.5F, 1, 0, 0, light);
                    vertexFrame(pose, vb, left, bottom, woodU_, woodV1, -0.5F, 1, 0, 0, light);
                    vertexFrame(pose, vb, left, bottom, woodU0, woodV1, 0.5F, 1, 0, 0, light);
                    vertexFrame(pose, vb, left, top, woodU0, woodV0, 0.5F, 1, 0, 0, light);
                }
                if (!fastRender) {
                    ...
                    if (renderWithImage) {
                        ...
                        rd = PaintingRendererManager.PixelsRenderer();
                    rd.render(stack, pose, buffers, entity.data, blocHor, blocVer, left, bottom, light);
                }
            }
        }
        if (fastRender) {
            ...
            if (renderWithImage) {
                ...
                rd = PaintingRendererManager.PixelsRenderer();
            rd.renderFull(stack, pose, buffers, entity.data, LEFT, TOP, light);
        }
        if (NekoPlatform.config().debugMode()) {
            // Draw Debug Text...
            stack.pushPose();
            stack.translate(-LEFT - 1.0D, TOP + 3.0D, -0.6D);
            stack.scale(-0.2F, -0.2F, 0.2F);
            font.drawInBatch((renderWithImage ? "Rendered with Image" : "Rendered Pixel-by-Pixel") + (fastRender ? " (Fast Mode)" : " (Fancy Mode)"), 1.0F, 1.0F, 0xFFFFFF, false, stack.last().pose(), buffers, Font.DisplayMode.NORMAL, 0, entityLight, false);
            stack.translate(0.0D, -10.0D, 0.0D);
            font.drawInBatch("#" + String.valueOf(entity.data.getPaintingHash()) + String.format(" L: %x", entityLight), 1.0F, 1.0F, 0xFFFFFF, false, stack.last().pose(), buffers, Font.DisplayMode.NORMAL, 0, entityLight, false);
            stack.translate(0.0D, 30.0D, 0.0D);
            font.drawInBatch(String.valueOf(entity.data.getUUID()), 1.0F, 1.0F, 0xFFFFFF, false, stack.last().pose(), buffers, Font.DisplayMode.NORMAL, 0, entityLight, false);
            stack.translate(0.0D, 10.0D, 0.0D);
            font.drawInBatch(String.valueOf(entity.getUUID()), 1.0F, 1.0F, 0xFFFFFF, false, stack.last().pose(), buffers, Font.DisplayMode.NORMAL, 0, entityLight, false);
            stack.popPose();
        }
    }

    private static void vertexFrame(PoseStack.Pose pose, VertexConsumer vertexBuilder, float x, float y, float u, float v, float z, int nx, int ny, int nz, int light) {
