...
- `PotBlock.canSustainPlant` (NeoForge `TriState`) was dropped; `FrameSideBlock` break-to-reconnect now goes through `playerWillDestroy`/`playerDestroy` (vanilla has no cancellable destroy hook); `CreativeModeTab` ordering is alphabetical by registry ID on both loaders: NeoForge's `CreativeModeTabRegistry.sortTabs()` topological sort and Fabric API's `ItemGroupsMixin.paginateGroups` both fall back to alphabetical order when no ordering constraints exist (Fabric has no ordering API at all, so `withTabsBefore` can't be restored cross-loader). Tab IDs are therefore chosen so alphabetical order equals display order (`nekoration_blocks_cement/stone/wooden`, `nekoration_decor_ornaments`); the `itemGroup.*` translation keys follow the IDs, so lang files must stay in sync when renaming. `FireBlock`-based flammability registration moved to the platform modules.
...
- Fabric has no per-model `render_type` support (that field is NeoForge-only and is otherwise ignored, leaving every block in the solid chunk layer). `fabric.client.FabricBlockRenderTypes` (registered as a client resource reload listener) scans the mod's blockstates + block models, resolves `render_type` through the model `parent` chains, and registers each affected block via Fabric's `BlockRenderLayerMap` (per block, strongest layer wins); NeoForge keeps reading the field natively.
