 ...
 *
 * Holds debugging settings for interactive debugging: Typical usage 1)
 * setDebugParameter(name, value) - for example in response to user command\
 * /mbedebug param <name> <value></value> /mbedebug param x 0.3 2)
 * getDebugParameter(name) - for example in a rendering routine double
 * renderOffsetX = getDebugParameter("x").orElse(0.0); which can then be
 * subsequently used for (eg) interactively adjusting rendering offsets in-game
 * 3) listAllDebugParameters() to be used for (eg) autocompletion suggestions is
 * automatically populated when code sets or gets a name 4) getDebugTest
 * triggers a debug test
 *
 * Also has parallel implementation for Vec3d instead of double.
 *
 * Also has parallel implementation for triggers (set the trigger, which is
 * automatically reset upon read)
 *
 * Note that this class only works properly for debugging on an integrated
 * server; i.e. the DebugSettings can be read from either the client or the
 * server side. There are no network messages to synchronise debug settings
 * between the server and multiple clients.
 *
 * In MinecraftByExample, DebugSettings is used in conjunction with the
 * following classes: MBEdebugCommand, which is registered in a
...
public class DebugSettings {
	...
	/**
	 * Checks the specified range of test numbers and returns the currently
	 * triggered test number if it lies within the checked range. If the currently
	 * triggered test lies within the checked range, it is reset Beware - if you
	 * have multiple callers looking for debug tests in the same range using
	 * getDebugTest, only the first caller will ever find one.
	 *
	 * eg: 1) caller calls setDebugTest(65) 2a) getDebugTest(0, 10) returns
	 * NO_TEST_TRIGGERED 2b) getDebugTest(60, 65) returns NO_TEST_TRIGGERED 2c)
	 * getDebugTest(60, 66) returns 65 2d) calling getDebugTest(60, 66) for a second
	 * time now returns NO_TEST_TRIGGERED
	 *
	 * @param testNumberMin
	 *            lowest test number to check
	 * @param testNumberMaxPlusOne
	 *            highest test number to check plus one
