...
public class EaselMenuBlockEnity extends TileEntity {
      ...
      for(ITextComponent itextcomponent : this.messages) {
         ...
         if (style != null && style.getClickEvent() != null) {
            ...
            if (clickevent.getAction() == ClickEvent.Action.RUN_COMMAND) {
               player.getServer().getCommands().performCommand(this.createCommandSourceStack((ServerPlayerEntity)player), clickevent.getValue());
            }
         }
      }
   ...
   public CommandSource createCommandSourceStack(@Nullable ServerPlayerEntity player) {
      ...
      ITextComponent itextcomponent = (ITextComponent)(player == null ? new StringTextComponent("EaselMenu") : player.getDisplayName());
      return new CommandSource(ICommandSource.NULL, Vector3d.atCenterOf(this.worldPosition), Vector2f.ZERO, (ServerWorld)this.level, 2, s, itextcomponent, this.level.getServer(), player);
   }
