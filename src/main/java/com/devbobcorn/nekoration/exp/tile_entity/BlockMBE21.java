...
/**
 * User: The Grey Ghost Date: 11/01/2015
 *
 * BlockTileEntityData is a simple block with an associated TileEntity. The base
 * block is shaped like a hopper, and the artifact hovering above it is rendered
 * in the TER.
 */
public class BlockMBE21 extends Block {
	...
	public void setPlacedBy(World worldIn, BlockPos pos, BlockState state, LivingEntity placer, ItemStack stack) {
		...
		if (tileentity instanceof TileEntityMBE21) { // prevent a crash if not the right type, or is null
			...
			Color[] colorChoices = { Color.BLUE, Color.CYAN, Color.YELLOW, Color.GREEN, Color.WHITE, Color.ORANGE,
					Color.RED };
	...
	// Create block shapes which match the shape of the hopper (copied from
	// HopperBlock)
	...
	private static final VoxelShape INSIDE_BOWL_SHAPE = Block.box(2.0D, 11.0D, 2.0D, 14.0D, 16.0D, 14.0D);
	private static final VoxelShape HOPPER_SHELL_SHAPE = VoxelShapes.join(INPUT_PLUS_MIDDLE_SHAPE, INSIDE_BOWL_SHAPE,
			IBooleanFunction.ONLY_FIRST);
	private static final VoxelShape BOTTOM_HUB_SHAPE = Block.box(6.0D, 0.0D, 6.0D, 10.0D, 4.0D, 10.0D);
