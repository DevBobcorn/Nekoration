 ...
 *
 * This class demonstrate manually drawing quads. The quads have position,
 * colour, texture, normal, overlay, and lightmap information: see
 * DefaultVertexFormats ENTITY = new
 * VertexFormat(ImmutableList.builder().add(POSITION_3F).add(COLOR_4UB).add(TEX_2F).add(TEX_2S).add(TEX_2SB).add(NORMAL_3B).add(PADDING_1B).build());
 * ENTITY = new VertexFormat(ImmutableList.builder() .add(POSITION_3F)
 * .add(COLOR_4UB) // 4 unsigned bytes of colour (R, G, B, alpha) .add(TEX_2F)
 * // 2 floats for texture uv .add(TEX_2S) // 2 shorts for overlay coordinates
 * .add(TEX_2SB) // 2 shorts for lightmap coordinates .add(NORMAL_3B) // 3 bytes
 * for normal vector .add(PADDING_1B).build());
 *
 * RenderType.getSolid() is also suitable if you're using a texture which has
 * been stitched into the block texture sheet. Textures can be stitched into the
 * texture sheet either 1) automatically, if you have specified them in the
 * block model for a registered block; or 2) manually, by adding it yourself
 * during TextureStitchEvent.Pre
 *
 * Other types of rendering are possible by producing custom RenderTypes (see
 * RenderTypeHelper)
 */
public class RenderQuads {
	...
	private static void drawCubeQuads(MatrixStack matrixStack, IRenderTypeBuffer renderBuffer, Color color,
		...
		IVertexBuilder vertexBuilderBlockQuads = renderBuffer
				.getBuffer(RenderType.entitySolid(MBE21_CUBE_FACE_TEXTURE));
