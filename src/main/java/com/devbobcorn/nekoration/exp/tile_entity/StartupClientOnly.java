...
public class StartupClientOnly {
	...
	public static void onClientSetupEvent(FMLClientSetupEvent event) {
		// Tell the renderer that the base is rendered using CUTOUT_MIPPED (to match the
		// Block Hopper)
		RenderTypeLookup.setRenderLayer(StartupCommon.blockMBE21, RenderType.cutoutMipped());
	...
	// // Stitch the cube texture into the block texture sheet so that we can use it
	// later for rendering.
	// Not needed for this example
	// @SubscribeEvent
	// public static void onTextureStitchEvent(TextureStitchEvent.Pre event) {
	//
	// // There are many different texture sheets; if this is not the right one, do
	// nothing
