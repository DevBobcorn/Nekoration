...
/**
 * This is a simple tile entity which stores information used by the associated
 * TileEntityRenderer (TER) to render the "artifact" floating above the hopper.
 * The information stored is: the render style: * wireframe (using lines) *
 * quads * entity model * wavefront the colour the angular position (only used
 * by wavefront model render example)
 *
...
public class TileEntityMBE21 extends TileEntity {
	...
	public double getNextAngularPosition(double revsPerSecond) {
		// we calculate the next position as the angular speed multiplied by the elapsed
		// time since the last position.
		// Elapsed time is calculated using the system clock, which means the animations
		// continue to
		...
		// where your tileEntity's update() method increments number_of_ticks_elapsed,
		// and partialTick is passed by vanilla
		...
		double nextAngularPosition = lastAngularPosition
				+ (timeNow - lastTime) * revsPerSecond * DEGREES_PER_REV / NANOSECONDS_PER_SECOND;
	...
	// In this case, we need it for the gem colour. There's no need to save the gem
	// angular position because
	...
	// entity unloads
	// In this case, we only need to store the gem colour. For examples with other
	// types of data, see MBE20
	@Override
	...
	public void load(BlockState blockState, CompoundNBT parentNBTTagCompound) {
		...
		// important rule: never trust the data you read from NBT, make sure it can't
		// cause a crash

		final int NBT_INT_ID = ExpNBTTypes.INT_NBT_ID;
		Color readArtifactColour = INVALID_COLOR;
		if (parentNBTTagCompound.contains("artifactColour", NBT_INT_ID)) { // check if the key exists and is an Int. You
																			// can omit this if a default value of 0 is
																			// ok.
	...
	/**
	 * Return an appropriate bounding box enclosing the TER This method is used to
	 * control whether the TER should be rendered or not, depending on where the
	 * player is looking. The default is the AABB for the parent block, which might
	 * be too small if the TER renders outside the borders of the parent block. If
	 * you get the boundary too small, the TER may disappear when you aren't looking
	 * directly at it.
	 * 
	...
	public enum EnumRenderStyle {
		...
		private static Optional<EnumRenderStyle> getEnumRenderStyleFromID(byte ID) {
			for (EnumRenderStyle enumRenderStyle : EnumRenderStyle.values()) {
				if (enumRenderStyle.nbtID == ID)
					return Optional.of(enumRenderStyle);
			}
			return Optional.empty();
		}

		EnumRenderStyle(int i_NBT_ID) {
