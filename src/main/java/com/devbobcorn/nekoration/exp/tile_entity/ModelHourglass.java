...
public class ModelHourglass extends Model {
	...
	public ModelHourglass() {
		...
		/*
		 * Construct the hourglass model by building it up from boxes The dimensions of
		 * the box are given in texels, each of which is 1/16 of minecraft world
		 * coordinates
		 * 
		 * e.g. a box which is 8x4x16 in size corresponds to a world block 0.5 x 0.25 x
		 * 1
		 * 
		 * For more information - see mbe80 and
		 * http://greyminecraftcoder.blogspot.com/2020/03/minecraft-model-1144.html
		 * 
		 * The solid parts of the hourglass are composed of the Top, Bottom, central
		 * ring, and two glass boxes. These are combined into one model. The rotation
		 * point is [0, 0, 0] for all added boxes, this is the centre of the ring in the
		 * middle. a setRotationPoint of [0, 24, 0] is added to counteract Vanilla's
		 * entity transformation upon rendering- i.e. all standard Vanilla models have a
		 * y offset of 24 units (1.5 blocks) to place their feet on the ground when
		 * rendered at y = 0. (see mbe80)
		 */
		...
		solidParts.addBox(TOP_POS.x(), TOP_POS.y(), TOP_POS.z(), TOP_SIZE.getX(), TOP_SIZE.getY(), TOP_SIZE.getZ(),
				EXPANSION_AMOUNT);

		solidParts.texOffs(BOTTOM_TEXTURE_ORIGIN.getLeft(), BOTTOM_TEXTURE_ORIGIN.getRight());
		solidParts.addBox(BOTTOM_POS.x(), BOTTOM_POS.y(), BOTTOM_POS.z(), BOTTOM_SIZE.getX(), BOTTOM_SIZE.getY(),
				BOTTOM_SIZE.getZ(), EXPANSION_AMOUNT);

		solidParts.texOffs(RING_TEXTURE_ORIGIN.getLeft(), RING_TEXTURE_ORIGIN.getRight());
		solidParts.addBox(RING_POS.x(), RING_POS.y(), RING_POS.z(), RING_SIZE.getX(), RING_SIZE.getY(),
				RING_SIZE.getZ(), EXPANSION_AMOUNT);

		solidParts.texOffs(TOP_GLASS_TEXTURE_ORIGIN.getLeft(), TOP_GLASS_TEXTURE_ORIGIN.getRight());
		solidParts.addBox(TOP_GLASS_POS.x(), TOP_GLASS_POS.y(), TOP_GLASS_POS.z(), TOP_GLASS_SIZE.getX(),
				TOP_GLASS_SIZE.getY(), TOP_GLASS_SIZE.getZ(), EXPANSION_AMOUNT);

		solidParts.texOffs(BOTTOM_GLASS_TEXTURE_ORIGIN.getLeft(), BOTTOM_GLASS_TEXTURE_ORIGIN.getRight());
		solidParts.addBox(BOTTOM_GLASS_POS.x(), BOTTOM_GLASS_POS.y(), BOTTOM_GLASS_POS.z(), BOTTOM_GLASS_SIZE.getX(),
				BOTTOM_GLASS_SIZE.getY(), BOTTOM_GLASS_SIZE.getZ(), EXPANSION_AMOUNT);
		...
		// * sand in the neck
		// These must be separate because we change their colour independently as the
		// sand runs from one chamber to the other
		...
		upperSand.addBox(UPPER_SAND_POS.x(), UPPER_SAND_POS.y(), UPPER_SAND_POS.z(), UPPER_SAND_SIZE.getX(),
				UPPER_SAND_SIZE.getY(), UPPER_SAND_SIZE.getZ(), EXPANSION_AMOUNT);
		...
		lowerSand.addBox(LOWER_SAND_POS.x(), LOWER_SAND_POS.y(), LOWER_SAND_POS.z(), LOWER_SAND_SIZE.getX(),
				LOWER_SAND_SIZE.getY(), LOWER_SAND_SIZE.getZ(), EXPANSION_AMOUNT);
		...
		neckSand.addBox(NECK_SAND_POS.x(), NECK_SAND_POS.y(), NECK_SAND_POS.z(), NECK_SAND_SIZE.getX(),
				NECK_SAND_SIZE.getY(), NECK_SAND_SIZE.getZ(), EXPANSION_AMOUNT);
		neckSand.setPos(0, 24, 0);
	}

	@Override
	public void renderToBuffer(MatrixStack ms, IVertexBuilder renderBuffer, int combinedLight, int combinedOverlay,
	...
	public void render(MatrixStack matrixStack, IVertexBuilder renderBuffer, int combinedLight, int combinedOverlay,
			float sandRed, float sandGreen, float sandBlue, float rotationDegrees, float upperSandFraction,
		...
		// draw sand first so that the semi-transparent glass blends properly over the
		// top of it

		// neck sand will match whichever of the sands is on top.
		// if rotation is between -90 to + 90 (>270 or <90) then the "upper" sand is on
		// top.
		...
		neckSand.render(matrixStack, renderBuffer, combinedLight, combinedOverlay, sandRed, sandGreen, sandBlue,
				neckSandAlpha);

		float upperSandAlpha = ALPHA_VALUE * upperSandFraction;
		upperSand.render(matrixStack, renderBuffer, combinedLight, combinedOverlay, sandRed, sandGreen, sandBlue,
				upperSandAlpha);

		float lowerSandAlpha = ALPHA_VALUE * lowerSandFraction;
		lowerSand.render(matrixStack, renderBuffer, combinedLight, combinedOverlay, sandRed, sandGreen, sandBlue,
				lowerSandAlpha);

		solidParts.render(matrixStack, renderBuffer, combinedLight, combinedOverlay, CONTAINER_RED, CONTAINER_GREEN,
				CONTAINER_BLUE, ALPHA_VALUE);
	}
}
