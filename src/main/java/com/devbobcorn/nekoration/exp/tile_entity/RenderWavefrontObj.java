...
/**
 * User: The Grey Ghost Date: 12/01/2015 This class shows how to render a
 * Wavefront OBJ model using a TileEntityRenderer
 *
 * Notes regarding the Wavefront obj file:
 *
 * 1) You need to manually replace the texture path in the .mtl file: * `map_Kd
 * mbe21_ter_gem.png` changes to `map_Kd minecraftbyexample:model/mbe21_ter_gem`
 * and you need to put the texture into assets/minecraftbyexample/textures/model
 * 2) You may need to define flip-v: true in your model.json if your textures
 * appear upside-down 3) You may need to set ambientToFullbright if your model
 * is always rendering at maximum brightness and is ignoring your combinedLight
 * method parameter.
 *
 * A breakpoint in OBJloader::loadModel() can help you troubleshoot wavefront
 * obj file problems
 */
public class RenderWavefrontObj {
	...
	public static void renderWavefrontObj(TileEntityMBE21 tileEntityMBE21, float partialTicks, MatrixStack matrixStack,
		...
		// We want the gem to be hovering with its bottom apex ([0,-1,0] in model space)
		// touching the middle of the top of the hopper
		...
		final Vector3d OBJ_MODEL_BOTTOM_APEX_WORLD_SPACE = new Vector3d(OBJ_MODEL_BOTTOM_APEX.x() * scaleX,
				OBJ_MODEL_BOTTOM_APEX.y() * scaleY, OBJ_MODEL_BOTTOM_APEX.z() * scaleZ);

		// translate model origin from [0,0,0] up to HOPPER_MIDDLE_OF_TOP and then
		// further to place the apex in the right place
	 ...
	 * When the player is a long distance away, the gem is dark, resting in the
	 * hopper, and does not rotate. // As the player approaches closer than 8
	 * blocks, the gem first starts to glow brighter // When the player gets closer
	 * than 6 blocks, the gem is at maximum brightness, and the gem starts to
	 * levitate above the pedestal // Once the player gets closer than 4 blocks, the
	 * gem reaches maximum height and starts to spin anti-clockwise // Once the
	 * player gets closer than 2 blocks, the gem reaches maximum spin speed
	 * 
	 * // the appearance and animation of the gem is hence made up of several parts:
	 * // 1) the colour of the gem, which is contained in the tileEntity (accounted
	 * for in renderWavefrontObj already) // 2) the brightness of the gem, which
	 * depends on player distance // 3) the distance that the gem rises above the
	 * pedestal, which depends on player distance // 4) the speed at which the gem
	 * is spinning, which depends on player distance.
	 */
	...
	private static void applyAnimations(TileEntityMBE21 tileEntityMBE21, MatrixStack matrixStack) {
		...
		double levitateHeight = UsefulFunctions.interpolate_with_clipping(playerDistance, DISTANCE_FOR_MIN_LEVITATE,
		...
		double revsPerSecond = UsefulFunctions.interpolate_with_clipping(playerDistance, DISTANCE_FOR_MIN_SPIN,
	 ...
	 * // The gem changes its appearance and animation as the player approaches. //
	 * When the player is a long distance away, the gem is dark, resting in the
	 * hopper, and does not rotate. // As the player approaches closer than 16
	 * blocks, the gem first starts to glow brighter // When the player gets closer
	 * than 8 blocks, the gem is at maximum brightness. // This method calculates
	 * the gem brightness (controlled by combinedLight = skylight + blocklight)
	 */
	@SuppressWarnings({ "resource" })
	private static int getCombinedLight(TileEntityMBE21 tileEntityMBE21, int ambientCombinedLight) {
		...
		double glowMultiplier = UsefulFunctions.interpolate_with_clipping(playerDistance, DISTANCE_FOR_MIN_GLOW,
				DISTANCE_FOR_MAX_GLOW, MIN_GLOW, MAX_GLOW);

		// change the "multitexturing" lighting value (default value is the brightness
		// of the tile entity's block)
