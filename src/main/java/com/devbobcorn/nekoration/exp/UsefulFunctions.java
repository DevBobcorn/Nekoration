...
/**
 * User: The Grey Ghost Date: 12/01/2015
 */
public class UsefulFunctions {
	/**
	 * linearly interpolate for y between [x1, y1] to [x2, y2] using x y = y1 + (y2
	 * - y1) * (x - x1) / (x2 - x1) For example: if [x1, y1] is [0, 100], and
	 * [x2,y2] is [1, 200], then as x increases from 0 to 1, this function will
	 ...
	 * @param y1
	 * @param y2
	 * @return linearly interpolated value. If x is outside the range, clip it to
	 *         the nearest end
	 */
	 ...
	 * 
	 * @param nbt
	 *            the Compound that holds the given tag
	 * @param tagname
	 *            name of the tag that was used to save the Vec3d
	 * @return the new Vec3d
	 */
	public static Vector3d deserializeVec3d(CompoundNBT nbt, String tagname) {
		ListNBT listnbt = nbt.getList(tagname, ExpNBTTypes.DOUBLE_NBT_ID);
