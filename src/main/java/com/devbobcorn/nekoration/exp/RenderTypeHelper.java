...
public final class RenderTypeHelper {
	// extract the private (protected) transparency settings so that we can create
	// custom RenderTypes with them.
	...
	public static final RenderType MBE_LINE_DEPTH_WRITING_ON; // draws lines which will only be drawn over by objects
																// which are closer (unlike RenderType.LINES)
	...
	static {
		LIGHTNING_TRANSPARENCY = ObfuscationReflectionHelper.getPrivateValue(RenderState.class, null,
				"field_228512_d_");
		...
		// PROJECTION_LAYERING =
		// ObfuscationReflectionHelper.getPrivateValue(RenderState.class, null,
		// "field_228500_J_");
		VIEW_OFFSET_Z_LAYERING = ObfuscationReflectionHelper.getPrivateValue(RenderState.class, null,
				"field_239235_M_");
		...
		renderState = RenderType.State.builder().setLineState(new RenderState.LineState(OptionalDouble.of(1)))
				.setLayeringState(VIEW_OFFSET_Z_LAYERING).setTransparencyState(NO_TRANSPARENCY)
				.setOutputState(ITEM_ENTITY_TARGET).setWriteMaskState(WRITE_TO_DEPTH_AND_COLOR)
		...
		renderState = RenderType.State.builder().setLineState(new RenderState.LineState(OptionalDouble.of(1)))
				.setLayeringState(VIEW_OFFSET_Z_LAYERING).setTransparencyState(NO_TRANSPARENCY)
				.setOutputState(ITEM_ENTITY_TARGET).setWriteMaskState(WRITE_TO_DEPTH_AND_COLOR)
				.setDepthTestState(NO_DEPTH_TEST).createCompositeState(AFFECTS_OUTLINE);
		MBE_LINE_NO_DEPTH_TEST = RenderType.create("mbe_line_1_no_depth_test", DefaultVertexFormats.POSITION_COLOR,
				GL11.GL_LINES, INITIAL_BUFFER_SIZE, renderState);

		renderState = RenderType.State.builder().setLayeringState(VIEW_OFFSET_Z_LAYERING)
				.setTransparencyState(NO_TRANSPARENCY).setOutputState(ITEM_ENTITY_TARGET)
				.setWriteMaskState(WRITE_TO_DEPTH_AND_COLOR).createCompositeState(AFFECTS_OUTLINE);
		MBE_TRIANGLES_NO_TEXTURE = RenderType.create("mbe_triangles_no_texture", DefaultVertexFormats.POSITION_COLOR,
				GL11.GL_TRIANGLES, INITIAL_BUFFER_SIZE, renderState);
	}
