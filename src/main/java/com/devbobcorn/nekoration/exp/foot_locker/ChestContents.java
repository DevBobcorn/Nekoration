 ...
 *
 * This class is used to encapsulate the contents of the chest and provide the
 * link between the parent TileEntity and the container. 1) stores information
 * about the items in the chest: allows the container to manipulate the data
 * stored in the tile entity 2) provides a way for the container to ask if
 * certain actions are permitted (eg isUsableByPlayer, isItemValidForSlot) 3)
 * provides a way for the container to notify the TileEntity that the container
 * has changed (eg markDirty, openInventory)
 *
 * Typical usage for a TileEntity which needs to store Items: 1) When
 * constructing the TileEntity, create and store a ChestContents using
 * createForTileEntity() 2) In your ContainerType<MyContainer>, create a
 * ChestContents using createForClientSideContainer() and pass it to the
 * constructor of your client-side container. 3) In your TileEntity write() and
 * read() methods, call the serializeNBT() and deserializeNBT() methods Vanilla
 * and the container code will take care of everything else.
 *
 */
...
public class ChestContents implements IInventory {
	...
	/**
	 * Use this constructor to create a ChestContents which is linked to its parent
	 * TileEntity. On the server, this link will be used by the Container to request
	 * information and provide notifications to the parent On the client, the link
	 * will be unused. There are additional notificationLambdas available; these two
	 * are explicitly specified because your TileEntity will nearly always need to
	 * implement at least these two
	 * 
	 * @param size
	 *            the max number of ItemStacks in the inventory
	 * @param canPlayerAccessInventoryLambda
	 *            the function that the container should call in order to decide if
	 *            the given player can access the container's contents not. Usually,
	 *            this is a check to see if the player is closer than 8 blocks away.
	 * @param markDirtyNotificationLambda
	 *            the function that the container should call in order to tell the
	 *            parent TileEntity that the contents of its inventory have been
	 *            changed and need to be saved. Usually, this is
	 *            TileEntity::markDirty
	 * @return the new ChestContents.
	 */
	public static ChestContents createForTileEntity(int size, Predicate<PlayerEntity> canPlayerAccessInventoryLambda,
	...
	/**
	 * Use this constructor to create a ChestContents which is not linked to any
	 * parent TileEntity; i.e. is used by the client side container: * does not
	...
	/**
	 * Writes the chest contents to a CompoundNBT tag (used to save the contents to
	 * disk)
	 * 
	...
	private ChestContents(int size, Predicate<PlayerEntity> canPlayerAccessInventoryLambda,
			Notify markDirtyNotificationLambda) {
		this.chestContents = new ItemStackHandler(size);
		this.canPlayerAccessInventoryLambda = canPlayerAccessInventoryLambda;
		this.markDirtyNotificationLambda = markDirtyNotificationLambda;
